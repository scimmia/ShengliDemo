 ...
 */
@RestController
@RequestMapping("/iosbingan")
public class IOSBinganController {
    private final static Logger log = LoggerFactory.getLogger(IOSBinganController.class);
    ...
    public @ResponseBody BaseResponse getBorrowList(
            ...
            @RequestParam(value="startindex", required=false, defaultValue="0") String startindex){
        log.info(DebugLog.info()+userid + "|||" + orgcode + "|||" + binganhao + "|||" + shenfenzhengid + "|||" +
                xingming + "|--" + Utils.getBase64Data(xingming) + "--|"  + chuyuanriqibegin + "|||" + chuyuanriqiend + "|||" + mubiaoorgcode + "|||"
                + requirecount + "|||" + startindex);
        List<Bingan> bingans = new BinganDao(jdbcTemplate).queryBorrowBingan(userid, orgcode, binganhao, shenfenzhengid,
                Utils.getBase64Data(xingming), chuyuanriqibegin, chuyuanriqiend, mubiaoorgcode, requirecount,startindex);
        return new BaseResponse<List<Bingan>>(0,null,bingans);
    }
    ...
    public @ResponseBody BaseResponse borrowBingan(
        ...
        log.info(DebugLog.info()+zuzhidaima + "|||" + zuzhiname + "|--" + Utils.getBase64Data(zuzhiname) + "--|"
                + binganid + "|||" + shenqingren+ "|||"
                +shenqingkeshicode + "|||" + shenqingkeshiname + "|--" + Utils.getBase64Data(shenqingkeshiname) + "--|"
                + shenqingdanweicode + "|||" + shenqingdanweiname+"|--" + Utils.getBase64Data(shenqingdanweiname) + "--|");
        int result = new BinganDao(jdbcTemplate).borrowBingan(
                zuzhidaima, Utils.getBase64Data(zuzhiname), binganid, shenqingren,shenqingkeshicode,
                Utils.getBase64Data(shenqingkeshiname), shenqingdanweicode, Utils.getBase64Data(shenqingdanweiname));
        String msg = "";
        switch (result){
            case 0:
